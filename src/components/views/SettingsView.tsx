...
interface SettingsViewProps {
  ...
  onUpdateUser?: (uid: string, name: string, role: UserRole, warehouseId: string, phone?: string) => Promise<void>;
...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const [authCode, setAuthCode] = useState<string>('admin123');
  ...
  const handleSaveUser = async (uid: string) => {
    ...
    try {
      ...
      setUserActionError('');
      if (onUpdateUser) {
        await onUpdateUser(uid, editingName.trim(), editingRole, editingWarehouseId, editingPhone.trim());
  ...
  const handlePurgeOtherUsers = async () => {
    ...
    try {
      ...
      for (const u of users) {
        const isSelf = u.uid === currentUserUid || u.email?.toLowerCase().trim() === 'chinarsales737@gmail.com';
