  ...
  const handleDeleteGroupedDispatch = async (dispatchNumber: string, itemsList: Array<{ id?: string }>) => {
    if (window.confirm(`Are you sure you want to permanently delete Dispatch Order "${dispatchNumber}" and reverse all of its ${itemsList.length} item line records?\n\nDeleting this series will automatically rearrange remaining series numbers sequentially.`)) {
      try {
        ...
        if (onRearrangeSeries) {
          await onRearrangeSeries();
        }
      } catch (err) {
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  alert(`Successfully auto-rearranged dispatch series! Updated ${res?.updatedCount || 0} order series numbers.`);
                } catch (e: any) {
              ...
              title="Auto Rearrange Dispatch Series sequentially (DSP-1001, DSP-1002...) to eliminate gaps"
            >
