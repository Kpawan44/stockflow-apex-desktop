...
import { StockMovement, AuditLog, UserRole, Product, Warehouse } from '../../types';
...
export const StockLedgerView: React.FC<StockLedgerViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'ledger' | 'item_wise_ledger' | 'audit' | 'import_logs'>('ledger');
  ...
  const itemWiseLedgerData = useMemo(() => {
    ...
    if (selectedWarehouseLedger !== 'All') {
      filtered = filtered.filter(m => 
        m.warehouseId === selectedWarehouseLedger || 
        m.fromWarehouseId === selectedWarehouseLedger || 
        m.toWarehouseId === selectedWarehouseLedger
      );
    }
  ...
  return (
    <div id="stock-ledger-view" className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        {/* Toggle between Ledger, Item-wise, Audit and Import Logs */}
        <div className="flex border border-gray-200 bg-gray-50/50 rounded-lg p-0.5 shadow-inner">
          <button
          ...
          >
            📊 Stock Ledger (Immutable Unit History)
          </button>
          <button
          ...
          >
            🛡️ Security Audit Logs (User Activity)
          </button>
          <button
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    alert(`Reconciliation Completed!\n\n- Cleared ${res.deletedMovementsCount} orphaned ledger entries.\n- Repaired ${res.correctedStocksCount} stock balance records.\n\nYour database is now fully verified and clean.`);
                  } catch (err: any) {
                ...
                title="Verify database consistency and self-heal orphaned entries"
              >
                  ...
                  {currentUserRole === 'Super Admin' && (
                    <button
                      onClick={async () => {
                        ...
                        if (window.confirm(`CRITICAL WARNING: Are you sure you want to completely delete item "${label}" (SKU: ${selectedItemCode})?\n\nThis will permanently delete all of its historical ledger/movement logs, and if it exists, its entry in the active Product Catalog.`)) {
                          try {
                            // Find and delete all movements for this item code
                            const movementsToDelete = movements.filter(m => m.itemCode === selectedItemCode);
                            ...
                            alert(`Successfully deleted SKU "${selectedItemCode}" entirely from movements ledger and product catalog.`);
                          } catch (err: any) {
                        ...
                        {currentUserRole === 'Super Admin' && (
                          <td className="p-4 text-right whitespace-nowrap">
                            <div className="flex items-center justify-end gap-1.5">
                              {(m.transactionType.includes('Adjustment') || m.referenceNumber.startsWith('ADJ-')) && !m.isReverted && !m.transactionType.includes('Reversal') && onRevertAdjustment && (
                                <button
                                  onClick={async () => {
                                    const confirmReason = window.prompt(
                                      `Revert Manual Adjustment (${m.referenceNumber}) for ${m.itemName}?\n\nThis will restore the previous stock balance and record an audit log.\n\nEnter reversal reason:`,
                                      `Reverted from Stock Ledger view by ${currentUserRole}`
                                    );
                                    if (confirmReason !== null) {
                                      try {
                                        await onRevertAdjustment(m.id || m.referenceNumber, confirmReason);
                                        alert(`Adjustment ${m.referenceNumber} successfully reverted and stock balance restored!`);
                                      } catch (e: any) {
                                        alert(`Failed to revert adjustment: ${e.message || e}`);
                                      }
                                    }
                                  }}
                                  ...
                                  title="Revert Manual Stock Adjustment & Restore Balance"
                                >
                              ...
                              <button
                                onClick={() => {
                                  if (window.confirm(`Are you sure you want to delete movement record "${m.referenceNumber}"? This action cannot be undone and may cause inventory mismatches.`)) {
                                    onDeleteMovement(m.id);
                                  }
                                }}
                                className="p-1 hover:text-rose-600 text-gray-400 bg-gray-50 rounded transition-colors cursor-pointer"
                      ...
                      <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        {currentUserRole === 'Super Admin' && (
                          <td className="p-4 text-right">
                            <button
                              onClick={() => {
                                  if (window.confirm("Are you sure you want to delete this specific security audit log entry?")) {
                        ...
                        <div className="flex items-center gap-3 sm:self-center">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {currentUserRole === 'Super Admin' && (
                              <button
                                onClick={() => {
                                  if (window.confirm("Are you sure you want to delete this specific bulk import log from history?")) {
                                    onDeleteAuditLog(log.id);
                                  }
                                }}
                                className="p-1.5 hover:text-rose-600 text-gray-400 hover:bg-rose-50 rounded transition-colors cursor-pointer"
                                title="Delete Import Audit Log"
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
                            )}
